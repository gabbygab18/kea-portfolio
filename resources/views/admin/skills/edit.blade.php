        ...
        <form method="POST" action="{{ route('admin.skills.update', $skill) }}" class="admin-form">
            ...
            <div class="form-group">
                ...
                <div class="icon-preview-box" id="editPreviewBox">
                    <span id="editIconPlaceholder" style="font-size:1.8rem;opacity:0.3">?</span>
                    <span id="editIconPreviewLabel">No icon selected</span>
                </div>
                <div class="icon-tabs" id="editIconTabs"></div>
                <input type="text" class="icon-search" id="editIconSearch"
...
@push('scripts')
    <script>
        ...
        // Renders icon into a container — preserves label span by NOT wiping innerHTML
        function renderIconEl(containerId, value, size = '1.8rem') {
            const container = document.getElementById(containerId);
            if (!container) return;
            // Remove only previously rendered icon (img or i), not the spans
            container.querySelectorAll('img, i').forEach(el => el.remove());
            const placeholder = container.querySelector('#' + containerId.replace('PreviewBox', 'IconPlaceholder'));
        ...
        // All functions now take IDs (strings), not elements
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const currentIcon = document.getElementById('editSkillIcon').value;

            if (currentIcon) {
                renderIconEl('editPreviewBox', currentIcon, '1.8rem');
                const label = document.getElementById('editIconPreviewLabel');
                if (label) label.textContent = currentIcon;
            }

            buildGrid('editIconGrid', 'editSkillIcon', 'editPreviewBox', 'editIconPreviewLabel', '', currentIcon, 'All');

            document.getElementById('editIconSearch').addEventListener('input', function () {
                const activeCat = document.querySelector('#editIconTabs .icon-tab.active')?.textContent.trim() || 'All';
                buildGrid('editIconGrid', 'editSkillIcon', 'editPreviewBox', 'editIconPreviewLabel',
                    this.value, document.getElementById('editSkillIcon').value, activeCat);
            });
        });
    </script>
@endpush
