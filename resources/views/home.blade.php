...
@section('content')
    <section class="hero">
        ...
        <img class="hero__deco hero__deco--cards" src="{{ asset('images/deco-cards.png') }}" alt="" aria-hidden="true" />
        <img class="hero__deco hero__deco--dice" src="{{ asset('images/deco-dice.png') }}" alt="" aria-hidden="true" />
        <img class="hero__deco hero__deco--rook" src="{{ asset('images/deco-rook.png') }}" alt="" aria-hidden="true" />
        <img class="hero__deco hero__deco--domino" src="{{ asset('images/deco-domino.png') }}" alt="" aria-hidden="true" />
    ...
    <section class="about">
        ...
        <div class="about__inner">
            <div class="about__image-wrap">
                <div class="about__card about__card--back">
                    <img src="{{ asset('images/queen-back.png') }}" alt="" />
                </div>
                <div class="about__card about__card--front">
                    <img src="{{ asset('images/queen-front.png') }}" alt="" />
                </div>
            </div>
        ...
        <div class="services__inner">
            ...
            <div class="services__grid" id="servicesGrid">
                ...
                @foreach($services as $service)
                    <div class="services__card" data-index="{{ $loop->index }}">
                        <div class="services__card-suit">
                            <span class="services__card-suit-symbol">{{ $suits[$loop->index % 4] }}</span>
                            <span class="services__card-suit-num">{{ $loop->iteration }}</span>
                        </div>
    ...
    <script>
        ...
        function buildTrack(filteredCards) {
            ...
            if (!filteredCards.length) {
                ...
                track.style.cssText = 'display:flex;align-items:center;justify-content:center;padding:2rem 1rem;width:100%;';
            ...
            if (filteredCards.length <= SCROLL_THRESHOLD) {
                ...
                outer.style.justifyContent = 'center';
                track.style.cssText = 'display:flex;flex-wrap:wrap;justify-content:center;gap:24px;transform:none;width:auto;padding:2rem 1rem;';
        ...
        document.addEventListener('click', e => {
            ...
            document.querySelectorAll(`.btn-heart[data-idx="${idx}"]`).forEach(b => {
                if (liked.has(idx)) b.classList.remove('liked');
                    ...
                    b.classList.remove('heart-bounce'); void b.offsetWidth;
                    b.classList.add('liked', 'heart-bounce');
                    b.addEventListener('animationend', () => b.classList.remove('heart-bounce'), { once: true });
                }
            });
        ...
        document.getElementById('commentOverlay').addEventListener('click', e => {
            if (e.target === document.getElementById('commentOverlay')) document.getElementById('commentOverlay').classList.remove('open');
        });
        ...
        function postComment() {
            ...
            comments[currentCardIdx].push({ initials: 'ME', name: '@you', text });
            input.value = ''; renderComments(); showToast('Comment posted!');
        }
        ...
        document.getElementById('shareOverlay').addEventListener('click', e => {
            if (e.target === document.getElementById('shareOverlay')) document.getElementById('shareOverlay').classList.remove('open');
        });
        ...
        document.addEventListener('click', e => {
            ...
            document.querySelectorAll(`.btn-save[data-idx="${idx}"]`).forEach(b => {
                if (saved.has(idx)) b.classList.remove('saved');
                    ...
                    b.classList.remove('save-bounce'); void b.offsetWidth;
                    b.classList.add('saved', 'save-bounce');
                    b.addEventListener('animationend', () => b.classList.remove('save-bounce'), { once: true });
                }
            });
        ...
        (function () {
            ...
            function getPositions() {
                return cards.map(c => {
                    const r = c.getBoundingClientRect(), gr = grid.getBoundingClientRect();
            ...
            function runShuffle() {
                ...
                const lockedH = grid.offsetHeight, lockedW = grid.offsetWidth;
                grid.style.height = lockedH + 'px';
                grid.style.minHeight = lockedH + 'px';
                grid.style.position = 'relative';
                section.style.overflow = 'hidden';
                const cx = lockedW / 2, cy = lockedH / 2;
                ...
                requestAnimationFrame(() => requestAnimationFrame(() => {
                    cards.forEach((card, i) => {
                        const p = positions[i];
                        card.style.transition = `transform ${STACK_DUR}ms cubic-bezier(0.4,0,0.2,1)`;
                        card.style.transform = `translate(${cx - p.x - p.w / 2}px, ${cy - p.y - p.h / 2}px) rotate(0deg)`;
                    });
                }));

                const SHUFFLE_ROUNDS = 4, SHUFFLE_DUR = 200;
                function shuffleRound(round) {
                    if (round >= SHUFFLE_ROUNDS) { dealOut(); return; }
                    cards.forEach((card, i) => {
                        const p = positions[i], dir = round % 2 === 0 ? 1 : -1;
                        const offset = (i - (cards.length - 1) / 2) * 8;
                        card.style.transition = `transform ${SHUFFLE_DUR}ms cubic-bezier(0.25,0.46,0.45,0.94)`;
                        card.style.transform = `translate(${cx - p.x - p.w / 2 + offset * dir}px, ${cy - p.y - p.h / 2 - Math.abs(offset) * 0.4}px) rotate(${dir * (Math.random() * 14 + 6)}deg)`;
                ...
                function dealOut() {
                    ...
                    cards.forEach((card, i) => {
                        setTimeout(() => {
                            card.style.transition = `transform ${DEAL_DUR}ms cubic-bezier(0.34,1.4,0.64,1)`;
                            card.style.transform = 'translate(0,0) rotate(0deg)';
                            card.style.zIndex = '';
                        }, i * 90);
                    });
            ...
            new IntersectionObserver((entries) => {
                entries.forEach(e => { if (e.isIntersecting) runShuffle(); });
